...
{
	public Vector3 FrogPosition, StartFrogPosition, FrogEndPosition;
	float Jump = 1.8f;
	int RotateFrog = 0;
//	public Animation animation;
	...
	public Vector3 riverBoatPosition;
	...
	void FollowTheModel()
	{
		float x = riverBoat.transform.position.x - riverBoatPosition.x;
		FrogPosition += new Vector3(x, 0, 0);

		riverBoatPosition = riverBoat.transform.position;
	}
	...
	{
		/*
         * If is a riverBoat log the frog have to stay on it, follow its movement
         */
		if (col.gameObject.tag == "RiverObject")
		{
			riverBoat = col.gameObject;
			riverBoatPosition = riverBoat.transform.position;
